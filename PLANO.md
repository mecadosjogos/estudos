...
**Card "Taxonomia da matéria", separado de "Rede de conceitos"** (decisão do usuário) — `network/graph.py::build_taxonomia_json`, só na tela da matéria. Mostra exclusivamente a camada de taxonomia (sem assunto/discriminação/coocorrência misturados, sem legenda de filtro — só existe uma camada), a união dos mapas de todas as aulas já geradas pelo termo compartilhado. "Rede de conceitos" continua sendo a visão completa com todas as camadas; este card é a hierarquia pura, sempre.
