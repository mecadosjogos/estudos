...
**Guia estruturado — banco em vez de markdown opaco.** Achado revisando o próprio guia contra a "Taxonomia da matéria" (fase 15): a árvore de conhecimento, presa dentro de uma string, não dava pra comparar com `Assunto` nem indexar, e a IA às vezes numerava o sumário sem numerar as seções correspondentes no corpo (numeração livre, nunca garantida em código). Virou campos estruturados em `LessonProcessingOutput` — `guia_titulo`, `guia_arvore` (floresta de `{rotulo, filhos}`, nunca raiz única forçada), `guia_secoes` (título + corpo, em ordem) e `guia_topicos` (sumário, só título — nunca número, sempre calculado em código a partir da posição). Persistidos como `Lesson.guia_titulo`/`guia_arvore_json` (regenerados por inteiro, sem `deriv_key` — mesmo motivo do resumo) e duas tabelas novas: `GuiaSecao` (idem, sem identidade própria) e `GuiaTopico` (com `deriv_key` por título normalizado, mesmo padrão de `Assunto` — reconciliado porque carrega o único estado editável de verdade: a correção manual de pra qual seção o item do sumário aponta, guardada pelo **título normalizado da seção-alvo, não pela posição**, porque `GuiaSecao` não tem identidade estável entre reprocessamentos). `Lesson.guia_md`/`guia_gerado_em` continuam existindo como **cache remontado por código** (`ai/guia_markdown.py`) a partir desses campos — `export/corpus.py`, `export/exam_export.py` e o botão "Exportar" (`/guia.md`) não mudaram nada.
