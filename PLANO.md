...
### 5b. Taxonomia da matéria

O mapa de taxonomia acima (fase 15) resolve a estrutura *dentro* de uma aula. Mas Direito mistura matérias entre si — "dever de cuidado" aparece em Penal e em Civil, um assunto atravessa disciplinas — e isso pedia uma visão além de uma aula só.

**"Rede de conceitos" foi tentada em três versões e removida — não serviu pro estudo real.** Registro pra não ser reproposta:
- **v1**: coocorrência textual bruta como fonte de aresta (bloco/aula/matéria). Denso demais nos níveis aula/matéria (qualquer par mencionado na mesma aula conectava) e impreciso no nível bloco (exemplo citando dois termos de passagem os conectava sem relação real).
- **v2**: fonte principal virou a taxonomia (`Lesson.mapa_mermaid` parseado e resolvido contra o glossário), coocorrência virou camada opcional, com legenda pra filtrar por tipo/origem. Ainda em vis-network (layout de força).
- **Por que caiu**: dois problemas que a v2 não resolveu. (1) Camadas "sempre calculadas" como assunto↔assunto ainda explodiam sozinhas — uma aula cobrindo 15 assuntos gera 105 arestas só entre eles, afogando a taxonomia visualmente mesmo escondida por padrão. (2) Mais fundamental: exigir que cada rótulo já fosse verbete aprovado no glossário (pra virar nó no grafo de Term.id) deixava a esmagadora maioria de qualquer mapa de taxonomia real invisível — um diagrama de ~35 nós só tinha 2-3 já cadastrados. Um grafo em layout de força, além disso, nunca ia se parecer com a árvore hierárquica real que a IA desenha. Testado em produção contra `/lessons/8/mapa` e comparado lado a lado: o resultado não tinha nenhuma relação visual com a fonte.

**O que ficou: card "Taxonomia da matéria", só na tela da matéria.** `glossary/mermaid.py::build_taxonomy_tree` funde o `mapa_mermaid` de todas as aulas da matéria numa árvore só, deduplicando nó pelo **rótulo normalizado** — não pelo id interno do Mermaid, que só vale dentro de uma aula. Igual às tentativas anteriores, **não exige verbete aprovado**: a estrutura aparece inteira desde o primeiro mapa gerado; `term_id` é um bônus condicional por nó, não um pré-requisito pra ele existir. `glossary/mermaid.py::build_taxonomia_tree_json` orquestra (busca as aulas, funde, devolve JSON).

**Renderização virou D3, não Mermaid** — pedido do usuário pra tornar o mapa navegável: zoom/pan livre (arrastar/rolar), clicar num nó expande/recolhe os filhos, e uma **câmera automática** reenquadra a tela a cada clique (nó clicado + todos os descendentes que acabaram de aparecer, ao expandir; nó clicado + seu pai, ao recolher). Mermaid é renderização estática, pensada pra documentação — não tem colapso de sub-árvore em tempo de execução nem controle de câmera programável; `d3.hierarchy` + `d3.tree` + troca de `children`/`_children` é o padrão clássico do D3 pra árvore hierárquica colapsável, e é isso que `static/taxonomia.js` implementa. Layout **esquerda-para-direita** (`tela_x = d.y`, `tela_y = d.x`, técnica padrão de árvore horizontal), porque rótulo jurídico é frase longa e top-down (o que o Mermaid fazia) ficava largo demais. Rótulo renderizado como `<foreignObject>` com uma `<div>` HTML (quebra linha via CSS normal) em vez de `<text>` SVG, que não quebra linha sem `<tspan>` manual. Vendorizado `static/d3.min.js` (bundle completo, sem CDN, mesmo padrão do resto do projeto).

Simplificação nova: se o mesmo conceito aparece com **pais diferentes** em aulas diferentes (raro), só o primeiro pai encontrado vira aresta — árvore colapsável exige um pai por nó.

**Extração de rótulo de nó aceita parêntese/colchete dentro de aspas** (`glossary/mermaid.py::_RE_MIOLO_ROTULO`) — rótulo jurídico com parêntese explicativo é comum (`"Normas jurídicas (códigos, leis)"`), e o regex original (herdado da fase 15, só reconhecia bracket sem aspas ou aspas sem bracket interno) lia o `(` de dentro do rótulo como se abrisse um segundo nó, corrompendo a extração e derrubando a aresta inteira em silêncio. Achado depurando por que a taxonomia real de produção (`/lessons/8/mapa`) tinha rótulos que sumiam do grafo mesmo já sendo verbete aceito.

Nó com `subject_ids` de mais de uma matéria ganha cor de destaque — o sinal de conceito-ponte; a cor vem de onde o termo aparece de verdade no texto (`network/graph.py::term_subject_ids`), não só de `Definition` (curadoria rara demais pra ser o único sinal). Não existe página própria: a rede aparece embutida na tela da matéria e na tela da aula. **Interação**: clique num nó destaca a vizinhança (resto escurece) e abre um painel ("abrir →") em vez de navegar direto; clique numa aresta mostra a origem e um link **"▸ ver aula"** por aula que a gerou, pra nunca deixar uma estrutura flutuando sem voltar à fonte, no mesmo espírito do "▸ ouvir o original" que atravessa o app. Arrastar nó já vem de graça do vis-network.

Visualização com **vis-network** vendorizado (`static/vis-network.min.js`, sem CDN — mesmo padrão do `mermaid.min.js`), física posiciona os nós sozinha. Implementação: `network/graph.py` (orquestrador) + `network/cooccurrence.py` (camada opcional) + `network/types.py` (dataclasses compartilhadas) + `glossary/mermaid.py::parse_taxonomy_edges`.
...
**Narração do guia de aula (TTS local via GPU).** Terceira peça, `tts-service/` — um serviço HTTP standalone (Chatterbox Multilingual v3, Resemble AI — trocado do F5-TTS original, que produzia áudio embaralhado em PT-BR mesmo depois de eliminada toda causa de configuração), fora do repositório-como-worker acima de propósito: API genérica ("texto entra, mp3 sai", sem nenhum conceito do Estudos), reusável por qualquer script na máquina, não só este app. Usa a voz embutida padrão do modelo — sem clonagem, por decisão explícita do usuário (nenhum dado de voz pessoal envolvido); `/synthesize` aceita um `speaker` opcional pra clonar a partir de `tts-service/vozes/<nome>/ref.wav` se algum dia fizer sentido, mas isso não é o caminho padrão. O worker fala com ele por HTTP (`worker/tts.py`), nunca importa o modelo direto. Um job `tts_guia` (mesma tabela `TranscriptionJob`, mesmo mecanismo de claim/heartbeat da transcrição, só um `target` novo) é enfileirado sozinho sempre que o guia estruturado de uma aula é (re)gerado; o worker narra cada `GuiaSecao` (título lido antes do corpo, pra marcar a virada de assunto ouvindo em sequência), concatena tudo num **mp3 só pra aula inteira** e comprime pra **32kbps mono** (mesmo padrão do áudio da aula gravada, `shared/audio.py::compress_to_mp3`, achado real: sem isso o mp3 saía no bitrate padrão do ffmpeg, bem mais pesado que necessário pra voz falada), com os timestamps de início/fim de cada seção enviados junto — a falha de uma seção não derruba as outras, só fica sem trecho na concatenação. A execução contínua do worker (`run_local.ps1`, sem `--target` fixo) drena transcrição e narração na mesma rodada, priorizando transcrição, ignora a fila de narração sem travar quando o `tts-service` não está de pé, e pede pro `tts-service` **encerrar o processo inteiro** (`POST /shutdown`) ao terminar de drenar essa fila, pra não deixar a GPU ocupada à toa entre uma leva e a próxima — achado real: `/unload` (que só larga o modelo, `_model = None` + `empty_cache()`) libera o grosso da VRAM, mas o contexto CUDA em si (kernels do cuDNN/cuBLAS, cache do alocador do PyTorch) só se solta quando o processo morre de verdade, deixando ~1-1.5GB presos à toa. O preço é que a próxima narração exige rodar `tts-service\iniciar.ps1` de novo — o serviço não fica mais de pé esperando entre uma leva e outra. Instalação separada dos outros dois processos (ver README.md, "Instalar em outra máquina") — `tts-service/instalar.ps1` isolado, ou o botão "Instalar narração" em `/admin/backups`; roda na mesma máquina que já tem `scripts/instalar_transcricao.ps1` instalado, já que é o worker em modo contínuo quem drena a fila de narração.
...
Estudos/
  ...
  tts-service/      main.py · instalar.ps1 · iniciar.ps1     serviço de TTS local (Chatterbox Multilingual v3), standalone
...
**Guia de aula — artefato extra, não previsto originalmente, complementar à aula editada.** Reorganiza a transcrição em seções legíveis preservando a voz do professor, sem os blocos tipados/`deriv_key` da aula editada — de propósito, porque esse conteúdo permite reordenar raciocínio dentro de uma seção (conceito → explicação → exemplo), o que quebraria a garantia de bloco = intervalo tocável que a aula editada carrega.

**A partir da fase 6 revisada: uma leitura só gera os dois.** Originalmente o guia nascia de uma **segunda chamada** (`server/app/ai/guia.py`, prompt próprio, pacote/colagem próprios em `/guia-pacote.md`/`/colar-guia`) — lendo a transcrição inteira de novo do zero. Isso foi corrigido: o guia virou mais campos de `LessonProcessingOutput` (junto de `resumo`, `aula_editada`, `cards`...), as instruções entraram em `bridge.py` ao lado das demais, e `_ingest()` persiste tudo na mesma passada que grava o resto. Sem chamada extra, sem pacote extra, sem colagem extra — `server/app/ai/guia.py` não existe mais. Ver "Decisões fechadas" para o porquê.

**Guia estruturado — banco em vez de markdown opaco, derivado por parser em vez de campos preenchidos pela IA.** Achado revisando o próprio guia contra a "Taxonomia da matéria" (fase 15): a árvore de conhecimento, presa dentro de uma string, não dava pra comparar com `Assunto` nem indexar, e a IA às vezes numerava o sumário sem numerar as seções correspondentes no corpo (numeração livre, nunca garantida em código). A primeira tentativa de correção trocou `guia_md: str` por campos estruturados na saída da IA (`guia_titulo`/`guia_arvore`/`guia_secoes`/`guia_topicos`); isso resolveu a numeração, mas piorou a qualidade da prosa — pré-decompor o texto em campos JSON isolados antes de escrever empurra a IA pra um modo "preencher formulário" em vez de compor um texto corrido. Revertido: a IA volta a escrever só `guia_md` (markdown corrido — título / árvore / sumário / corpo por seções / trechos incompletos, ver `ai/bridge.py`), e um **parser em código** (`ai/guia_parser.py`, zero chamada de IA extra) interpreta os mesmos cabeçalhos que a IA já usa naturalmente pra derivar título, árvore (floresta de `{rotulo, filhos}`, nunca raiz única forçada), seções (título + corpo, em ordem) e sumário (só título — nunca número, sempre calculado em código a partir da posição). Persistidos como `Lesson.guia_titulo`/`guia_arvore_json` (regenerados por inteiro, sem `deriv_key` — mesmo motivo do resumo) e duas tabelas: `GuiaSecao` (idem, sem identidade própria) e `GuiaTopico` (com `deriv_key` por título normalizado, mesmo padrão de `Assunto` — reconciliado porque carrega o único estado editável de verdade: a correção manual de pra qual seção o item do sumário aponta, guardada pelo **título normalizado da seção-alvo, não pela posição**, porque `GuiaSecao` não tem identidade estável entre reprocessamentos). `Lesson.guia_md`/`guia_gerado_em` continuam existindo como **cache remontado por código** (`ai/guia_markdown.py`) a partir do que o parser derivou — `export/corpus.py`, `export/exam_export.py` e o botão "Exportar" (`/guia.md`) não mudaram nada.

Árvore vira também **checklist leve contra `Assunto`**: nós-folha sem `Assunto` aceito correspondente pra aquela aula ganham destaque visual na tela — só folhas, porque nó intermediário ("Direito público") é categoria de organização, nunca um `Assunto` de verdade.

**Aulas processadas antes dessa mudança não precisam de migração nenhuma.** A tela `/guia` detecta se existe `GuiaSecao` pra aquela aula: se não existir, mostra o guia do jeito que sempre foi (`guia_legado.html`, servindo o `guia_md` antigo); só reprocessar de verdade faz a aula "subir" pra tela nova (sumário navegável, árvore, checklist). Sem script de backfill — o `guia_md` antigo nunca é perdido nem convertido, só deixa de alimentar a tela nova até você reprocessar.
...
20b. **Taxonomia da matéria**: numa matéria com duas ou mais aulas com `mapa_mermaid` gerado, conferir que a árvore na tela da matéria traz os nós das duas aulas, que o nó repetido entre elas aparece uma vez só (não duplicado). Clicar num nó com filhos: recolhe/expande, a câmera reenquadra (nó + descendentes ao expandir, nó + pai ao recolher). Clicar na setinha "→" de um nó com verbete navega pra `/termos/{id}` sem recolher o nó. Arrastar e rolar funcionam como pan/zoom manual. Numa matéria sem nenhum mapa gerado ainda, conferir a mensagem "nenhuma aula... ainda" em vez de tela vazia. Numa matéria com árvore grande (30+ nós), confirmar que dá pra ler qualquer rótulo dando zoom, ao contrário do Mermaid estático anterior (que espremia tudo pra caber na largura do card, ficando ilegível — bug relatado em produção que motivou a troca pra D3).
...
| **Guia de aula virou campos estruturados, não markdown opaco** | `guia_md` como string única escondia a árvore de conhecimento de qualquer reaproveitamento (não dava pra comparar com `Assunto`, indexar, nem garantir numeração consistente entre sumário e corpo — a IA às vezes numerava um e não o outro, porque numeração era texto livre). Virou `guia_titulo`/`guia_arvore`/`guia_secoes`/`guia_topicos` em `LessonProcessingOutput`, persistidos em colunas de `Lesson` + tabelas `GuiaSecao`/`GuiaTopico`. `guia_md` continua existindo, mas como cache remontado por código (`ai/guia_markdown.py`), não mais escrito direto pela IA |
| **Árvore de conhecimento do guia é coluna JSON, não tabela auto-referenciada** | Só é lida por inteiro (renderização + checklist contra `Assunto`), nunca consultada nó a nó via SQL — uma tabela com pai/filho seria migração e código a mais sem uso relacional real |
| **Correção manual do sumário-alvo guardada por título normalizado, não por posição** | `GuiaSecao` não tem `deriv_key`/identidade estável entre reprocessamentos (apaga e recria por inteiro) — se `GuiaTopico.secao_alvo_slug` guardasse um número de posição cru, um reprocessamento que reordenasse as seções faria a correção apontar pra seção errada, silenciosamente. A tela continua mostrando um campo numérico (a tradução pra slug acontece no POST); quando o slug não bate mais com nenhuma seção atual, a tela avisa "seção-alvo desatualizada" em vez de linkar errado sem avisar |
| **Checklist árvore × `Assunto` só marca nós-folha, por slug exato** | Nó intermediário ("Direito público") é categoria de organização que o professor usou, nunca vira um `Assunto` de verdade — marcá-lo também gerar destaque constante e inútil. Casamento fuzzy (`similar_slugs`) foi descartado de propósito: mais simples e evita marcar errado por aproximação; recalibra só se sobrar ruído real de uso |
| **Guia estruturado não tem script de backfill — fallback por template, não migração de dado** | Converter o `guia_md` antigo (parsing de texto) exigiria escrever e validar um parser só pra popular estrutura que ninguém pediu que fosse retroativa. A tela detecta ausência de `GuiaSecao` e renderiza `guia_legado.html` (cópia intocada do template antigo) — a aula só ganha a experiência nova quando reprocessada de verdade, sem apagar nem converter nada do que já existia |
...
| **"Rede de conceitos" (grafo livre Termo+Assunto, vis-network) foi removida** | Três versões tentadas (coocorrência bruta → taxonomia parseada + camadas opcionais), todas em layout de força. Nunca ficou utilizável pro estudo real: camadas "sempre calculadas" explodiam sozinhas (uma aula com 15 assuntos gera 105 arestas só entre eles) e exigir verbete aprovado pra virar nó deixava a maioria de qualquer taxonomia real invisível. Decisão do usuário, depois de comparar lado a lado com `/lessons/{id}/mapa`: manter só o card "Taxonomia da matéria" (Mermaid fundido, não vis-network — ver seção 5b). Registrado pra não ser reproposta sem antes ler o histórico da seção 5b |
| **Taxonomia da matéria funde pelo rótulo, sem exigir verbete aprovado** | Diferente de tudo que a "Rede de conceitos" tentava (grafo de `Term.id`, exigindo resolução contra o glossário pra existir), `build_taxonomy_tree` funde pelo texto do rótulo em si. A estrutura do mapa de taxonomia é o produto; `term_id` é um bônus condicional, não um pré-requisito pro nó existir |
| **Taxonomia da matéria renderiza com D3, não Mermaid** | Mermaid estático (primeira versão do card) espremia o diagrama inteiro pra caber na largura do card sem zoom nem scroll — um mapa de ~35 nós renderizava a ~24% do tamanho, ilegível (bug relatado em produção: "Direito privado" parecia ter sumido, mas estava lá, só minúsculo). D3 (`d3.hierarchy`/`d3.tree`, árvore colapsável clássica) dá zoom/pan de verdade, esquerda-para-direita (rótulo jurídico é frase longa, top-down ficava largo demais), com câmera automática reenquadrando a cada clique |
