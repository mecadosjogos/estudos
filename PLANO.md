...
Nós = Termos do glossário + Assuntos. Camadas de aresta, todas agrupadas **por aula individual** (nunca fundem aulas diferentes num grupo só, nem na tela da matéria — é o que evita a bola de pelo da v1):
- **Taxonomia** (sempre ligada) — `parse_taxonomy_edges`, dirigida (a seta carrega o sentido que a IA desenhou), com rótulo opcional (`-->|rótulo|`).
- **Cards de discriminação** (sempre ligada, fase 8b) — pares que o professor comparou explicitamente na fala. Aresta de alta precisão, com rótulo (eixo da distinção).
- **Assunto↔Assunto e Termo↔Assunto** (sempre ligada) — via `LessonAssunto`, dois assuntos conectam só se a mesma aula cobre os dois.
- **Coocorrência textual bruta** (opcional, desligada por padrão) — `glossary/matcher.py::find_matches` sobre `EditedBlock.texto`, pra quem quiser ver também o que apareceu perto no texto sem estrutura desenhada. Nível bloco não existe mais (era o mais impreciso).

O servidor sempre manda o grafo com as quatro camadas já calculadas (`incluir_coocorrencia=True` fixo em `build_rede_json`) — o filtro de quais aparecem é **100% client-side**, numa legenda (`static/rede.js::buildLegend`) que ao mesmo tempo explica e liga/desliga: um checkbox por tipo de nó (círculo=termo, losango=assunto) e um por origem de aresta (linha sólida com seta=taxonomia, tracejada=distinção, sólida=assunto, fina=coocorrência), sem round-trip ao servidor quando você muda o filtro.
