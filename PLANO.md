...
O mapa de taxonomia acima resolve a estrutura *dentro* de uma aula. Mas Direito mistura matérias entre si — "dever de cuidado" aparece em Penal e em Civil, um assunto atravessa disciplinas — e isso pede uma visão **livre, sem hierarquia forçada**, entre aulas e entre matérias. Diferente do mapa de taxonomia, a rede de conceitos **não usa IA** para montar as conexões — mesmo depois da v2 (ver abaixo), continua sem chamada de IA nova.

**v1 (coocorrência bruta como fonte principal) foi tentada e revertida.** Dois termos que apareceram perto um do outro no texto viravam aresta, em três escopos (bloco/aula/matéria). Testado em produção, deu dois problemas: nos níveis aula e matéria virou denso demais pra visualizar (quantidade de conexão ≠ estrutura da matéria — qualquer par mencionado na mesma aula conectava), e no nível bloco ficou impreciso (um exemplo citando dois termos de passagem os conectava sem relação nenhuma entre eles).

**v2 (atual): a fonte principal de aresta Termo↔Termo é a taxonomia**, não coocorrência. `glossary/mermaid.py::parse_taxonomy_edges` parseia o `Lesson.mapa_mermaid` que a IA já desenha por aula (fase 15) — um diagrama *curado*, não uma lista de tudo que foi mencionado — e resolve cada nó pelo **rótulo casado contra o glossário** (não pelo id interno do Mermaid, que só vale dentro de um diagrama). É esse casamento por rótulo que une o mapa de aulas diferentes num grafo só: cada diagrama sozinho é uma arvorezinha, mas um termo que reaparece em diagramas de aulas diferentes acaba com mais de um vizinho vindo de fontes diferentes — a união vira rede livre, não árvore, de graça.

Nós = Termos do glossário + Assuntos. Camadas de aresta, todas calculadas sempre e agrupadas **por aula individual** (nunca fundem aulas diferentes num grupo só, nem na tela da matéria — é o que evita a bola de pelo da v1), mas só duas vêm **visíveis por padrão** — as outras duas ficam calculadas e prontas, só escondidas até você marcar:
- **Taxonomia** (visível por padrão) — `parse_taxonomy_edges`, dirigida (a seta carrega o sentido que a IA desenhou), com rótulo opcional (`-->|rótulo|`). É a fonte principal, por isso a única sempre visível de cara junto com discriminação.
- **Cards de discriminação** (visível por padrão, fase 8b) — pares que o professor comparou explicitamente na fala. Aresta de alta precisão, com rótulo (eixo da distinção).
- **Assunto↔Assunto e Termo↔Assunto** (calculada sempre, escondida por padrão) — via `LessonAssunto`, dois assuntos conectam só se a mesma aula cobre os dois. Continua agrupada por aula individual, mas mesmo assim uma aula que cobre muitos assuntos de uma vez explode sozinha (par-a-par: 15 assuntos numa aula = 105 arestas só entre eles) e afogava visualmente a taxonomia por baixo — por isso escondida até você marcar.
- **Coocorrência textual bruta** (calculada sempre, escondida por padrão) — `glossary/matcher.py::find_matches` sobre `EditedBlock.texto`, pra quem quiser ver também o que apareceu perto no texto sem estrutura desenhada. Nível bloco não existe mais (era o mais impreciso).

O servidor sempre manda o grafo com as quatro camadas já calculadas (`incluir_coocorrencia=True` fixo em `build_rede_json`) — o filtro de quais aparecem é **100% client-side**, numa legenda (`static/rede.js::buildLegend`) que ao mesmo tempo explica e liga/desliga: um checkbox por tipo de nó (círculo=termo, losango=assunto) e um por origem de aresta (linha sólida com seta=taxonomia, tracejada=distinção, sólida=assunto, fina=coocorrência), sem round-trip ao servidor quando você muda o filtro. Termo que participa de pelo menos uma aresta de taxonomia vira **triângulo** em vez de círculo (`static/rede.js::formaDoNo`) — sinal visual de "isso está na hierarquia", independente do filtro (desmarcar a camada esconde a linha, não desfaz o triângulo). Esconder uma camada também esconde o nó que só tinha aresta daquela origem (`static/rede.js::baseFiltrada`) — sem isso, um nó cuja única conexão era "assunto" continuava aparecendo solto na tela mesmo com a camada desmarcada, diluindo visualmente a estrutura que sobrou.

**Card "Taxonomia da matéria", separado de "Rede de conceitos"** (decisão do usuário) — só na tela da matéria. Primeira tentativa renderizou isso com vis-network (mesmo motor da "Rede de conceitos") e não deu certo: layout de força não lembra em nada a árvore hierárquica que a IA desenha, e exigir que cada rótulo já fosse verbete aprovado (`taxonomy_edges`, que serve a "Rede de conceitos") deixava a esmagadora maioria da estrutura invisível — um mapa de ~35 nós só tinha 2-3 já cadastrados no glossário. Reescrito pra reusar o **mesmo motor visual do mapa por aula** (`static/mermaid.min.js`, mesmo template pattern de `mapa.html`): `glossary/mermaid.py::merge_taxonomy_diagrams` funde o `mapa_mermaid` de todas as aulas num diagrama Mermaid só, deduplicando nó pelo **rótulo normalizado** (não pelo id interno, que só vale dentro de uma aula) e **sem exigir verbete aprovado** — a estrutura aparece inteira desde o primeiro mapa gerado; só o `click` pro verbete (via `link_mermaid_nodes_to_glossary`, reaproveitado) depende de aprovação, igual ao mapa por aula. `network/graph.py::build_taxonomia_mermaid` orquestra. "Rede de conceitos" continua sendo a visão completa com todas as camadas em vis-network; este card é a hierarquia pura, sempre, no motor que sabe desenhar hierarquia.
...
20b. **Rede de conceitos**: numa aula com `mapa_mermaid` gerado, confirmar que a aresta de taxonomia aparece com seta (direção preservada) e, se o mesmo par se repete no mapa de outra aula da matéria, que o peso soma e a aresta acumula as duas aulas de origem. Clicar num nó: vizinhança destaca, resto escurece, painel mostra "abrir →". Clicar numa aresta de taxonomia: painel mostra "▸ ver aula" linkando pra cada aula de origem. Na legenda, desmarcar "Termo" e confirmar que os círculos (e as arestas que dependiam deles) somem, sem recarregar a página; marcar "Coocorrência de texto" (começa desmarcada) e confirmar que aparecem arestas finas extras. Confirmar que um termo usado em mais de uma matéria aparece com a cor de destaque mesmo dentro da tela de uma matéria só, e que pelo menos um par de discriminação aparece tracejado com o eixo da distinção no painel.
...
| **Rede de conceitos é script, não IA** | Pedir pra IA comparar cada termo contra o glossário inteiro pra propor conexão tem custo O(n²) que cresce sem controle conforme o glossário cresce. Taxonomia parseada (`glossary/mermaid.py::parse_taxonomy_edges`) e cards de discriminação já existentes (fase 8b) cobrem o problema a custo zero/quase-zero, sem IA nova — vale pra v1 (coocorrência) e pra v2 (taxonomia). Ver seção "5b. Rede de conceitos" |
| **Rede de conceitos é contextual, sem página/rota própria** | Cogitado uma página `/rede` isolada com seletor de escopo; o uso real é sempre "quero ver a rede desta matéria" ou "desta aula", nunca a rede solta sem contexto. Embutida em `subject_detail.html` e `lesson_detail.html` — decisão do usuário |
| **Rede de conceitos usa taxonomia, não coocorrência bruta, como fonte principal de aresta Termo↔Termo** | v1 usava coocorrência textual (`find_matches` sobre `EditedBlock.texto`) como fonte principal, em três escopos (bloco/aula/matéria). Testado em produção: nos níveis aula/matéria virou denso demais pra visualizar (quantidade de conexão ≠ estrutura da matéria), e no nível bloco ficou impreciso (exemplo citando dois termos de passagem os conectava sem relação real). v2 usa o mapa de taxonomia (`Lesson.mapa_mermaid`, já existente da fase 15) como fonte principal — estrutura que a IA desenhou, não proximidade de texto — com coocorrência rebaixada a camada opcional, desligada por padrão, e nível bloco removido de vez |
| **Arestas da rede sempre agrupadas por aula individual, nunca por matéria inteira** | Fundir os assuntos/termos de todas as aulas de uma matéria num grupo só (como a v1 fazia no escopo "materia") era o que produzia a explosão de arestas. Fixar o agrupamento em "por aula" mesmo quando a chamada cobre a matéria inteira mantém a densidade sob controle nos três sinais que dependem de agrupamento (assunto↔assunto, termo↔assunto, coocorrência opcional) — só a taxonomia funde entre aulas, e funde pelo termo compartilhado, não por espaço geográfico do grafo |
| **Filtro de camada da rede é legenda client-side, não parâmetro de rota** | Cogitado um checkbox por camada recarregando a página, ou um parâmetro `?camadas=...` na URL. O servidor já manda o grafo inteiro (custo de computar coocorrência é barato — mesmo raciocínio de sempre), então filtrar no cliente é instantâneo e dá de graça um efeito colateral bom: a legenda explica cada forma/traço no mesmo lugar onde liga/desliga, sem duas UIs separadas (uma que explica, outra que filtra) |
