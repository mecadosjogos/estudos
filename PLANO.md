...
- **Coocorrência textual bruta** (opcional, desligada por padrão — checkbox "+ coocorrência bruta") — `glossary/matcher.py::find_matches` sobre `EditedBlock.texto`, pra quem quiser ver também o que apareceu perto no texto sem estrutura desenhada. Nível bloco não existe mais (era o mais impreciso).

Nó com `subject_ids` de mais de uma matéria ganha cor de destaque — o sinal de conceito-ponte; a cor vem de onde o termo aparece de verdade no texto (`network/graph.py::term_subject_ids`), não só de `Definition` (curadoria rara demais pra ser o único sinal). Não existe página própria: a rede aparece embutida na tela da matéria e na tela da aula. **Interação**: clique num nó destaca a vizinhança e abre um painel ("abrir →") em vez de navegar direto; clique numa aresta mostra a origem e um link **"▸ ver aula"** por aula que a gerou, pra nunca deixar uma estrutura flutuando sem voltar à fonte, no mesmo espírito do "▸ ouvir o original" que atravessa o app. Arrastar nó já vem de graça do vis-network.
...
20b. **Rede de conceitos**: numa aula com `mapa_mermaid` gerado, confirmar que a aresta de taxonomia aparece com seta (direção preservada) e, se o mesmo par se repete no mapa de outra aula da matéria, que o peso soma e a aresta acumula as duas aulas de origem. Clicar num nó: vizinhança destaca, resto escurece, painel mostra "abrir →". Clicar numa aresta de taxonomia: painel mostra "▸ ver aula" linkando pra cada aula de origem. Marcar "+ coocorrência bruta" e confirmar que aparecem arestas extras sem recarregar a página, e desmarcar volta ao grafo esparso. Confirmar que um termo usado em mais de uma matéria aparece com a cor de destaque mesmo dentro da tela de uma matéria só, e que pelo menos um par de discriminação aparece tracejado com o eixo da distinção no painel.
