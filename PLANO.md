...
## Começar em uma sessão nova

Este plano é autossuficiente. Para executar, abra um chat novo na pasta do projeto e aponte para ele pelo caminho completo:

> `Leia C:\Users\wyver\.claude\plans\quero-criar-um-programa-velvet-sun.md e execute a fase 0.`

A fase 0 copia este arquivo para `PLANO.md` na raiz do projeto e cria um `CLAUDE.md` apontando para ele. **Da fase 1 em diante, basta dizer "execute a fase N do PLANO.md"** — não é mais preciso lembrar do caminho, e o plano passa a ser versionado junto com o código.

Vale ir por fases, conferindo cada uma antes da seguinte — em especial a **6** (IA, recorte de contexto e ponte manual) e a **13** (Feynman e dissertativa), que dependem mais de calibração do que de código.

**A entrega A inteira (fases 0–5) não depende de nada externo além do DNS** e pode começar hoje: ao fim dela você já grava, sobe, transcreve, ouve e busca. A chave da API só é necessária na fase 6; a ementa e as datas de prova, só na 14.
