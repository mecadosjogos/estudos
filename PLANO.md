...
**O que ficou: card "Taxonomia da matéria", só na tela da matéria.** Reusa o **mesmo motor visual do mapa por aula** (`static/mermaid.min.js`, mesmo template pattern de `mapa.html`) em vez de vis-network — layout de força nunca ia lembrar hierarquia, o dagre do próprio Mermaid é que sabe desenhar árvore. `glossary/mermaid.py::merge_taxonomy_diagrams` funde o `mapa_mermaid` de todas as aulas da matéria num diagrama Mermaid só, deduplicando nó pelo **rótulo normalizado** — não pelo id interno do Mermaid, que só vale dentro de uma aula. Ao contrário das tentativas anteriores, **não exige verbete aprovado**: a estrutura aparece inteira desde o primeiro mapa gerado; só o `click` pro verbete (via `link_mermaid_nodes_to_glossary`, reaproveitado) depende de aprovação, igual ao mapa por aula. `glossary/mermaid.py::build_taxonomia_mermaid` orquestra (busca as aulas, funde, liga ao glossário).
...
20b. **Taxonomia da matéria**: numa matéria com duas ou mais aulas com `mapa_mermaid` gerado, conferir que o diagrama fundido na tela da matéria traz os nós das duas aulas, que o nó repetido entre elas aparece uma vez só (não duplicado), e que os cliques levam ao verbete certo quando o rótulo já é um termo aprovado. Numa matéria com só uma aula com mapa, conferir que o diagrama fundido é visualmente idêntico ao `/lessons/{id}/mapa` daquela aula. Numa matéria sem nenhum mapa gerado ainda, conferir a mensagem "nenhuma aula... ainda" em vez de tela vazia.
...
| **Taxonomia da matéria funde pelo rótulo, sem exigir verbete aprovado** | Diferente de tudo que a "Rede de conceitos" tentava (grafo de `Term.id`, exigindo resolução contra o glossário pra existir), `merge_taxonomy_diagrams` funde pelo texto do rótulo em si. A estrutura do mapa de taxonomia é o produto; o `click` pro verbete é um bônus condicional, não um pré-requisito pra aresta existir |
