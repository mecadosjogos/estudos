...
**Narração do guia (TTS local, `tts-service/`)** é uma quarta peça, à parte do server/worker acima: serviço HTTP standalone (Coqui XTTS v2) na máquina com GPU, instalado com `tts-service\instalar.ps1` e ligado com `tts-service\iniciar.ps1` (fica escutando em `127.0.0.1:8100`, processo separado, precisa estar de pé pra qualquer coisa relacionada a narração funcionar). O worker (`worker/tts.py`) só fala HTTP com ele — nunca importa o modelo direto — e o modo contínuo padrão do worker (sem `--target` fixo) já drena a fila de narração sozinho quando a de transcrição está vazia, sem comando manual extra. Ver PLANO.md, seção "Arquitetura", pro desenho completo.
